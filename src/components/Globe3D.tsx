// Vendored from Aceternity UI's 3d-globe component
...
export interface GlobeMarker {
  ...
  src: string;
  label?: string;
  /** Per-marker diameter override, in CSS pixels */
  size?: number;
}
...
// Self-hosted copies of the NASA Blue Marble textures the upstream component
// fetches from a CDN at runtime. See scripts/fetch-globe-textures.sh.
...
function RotatingGlobe({
  ...
  // Load and configure the Earth textures. Upstream mutated the textures in a
  // `useMemo` after loading; doing it in useTexture's own callback keeps the
  // mutation inside the hook that constructs them.
...
export function Globe3D({
  ...
  return (
    // No default height: upstream hardcoded `h-[500px]` here, which left two
    // competing height utilities on the element whenever a caller passed its
    // own. The caller owns the height.
